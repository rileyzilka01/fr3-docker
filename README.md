...
Make sure you go into the docker file and set USER_UID and USER_GID to the respective IDs for your current user.
...
```bash
docker exec -it fr3-docker bash
```
...
After building there will be CMake warnings but they're non fatal and should be a successful build.
...
Add the line to CMakeLists.txt in the package you added if it does not yet exist
```
file(GLOB PY_SCRIPTS "src/*.py")
...
# Running the example go to zero script
```bash
